<template>
    <div class="container-fluid">
            ...
            <div class="card-body">
                <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                    <table class="table dataTable my-0" id="dataTable">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in ModoPagos" :key="index">
                                <td>{{ index + 1 }}</td>
                                <td>{{ item.nombre }}</td>
                                <td>
        ...
        <div class="card shadow">
            <div class="card-body">
                    <form @submit.prevent="guardar(ModoPago)">
                        <div class="form-row">
                            <div class="form-group col-md-12">
                                <input type="hidden" class="form-control" id="id" required  v-model="ModoPago.id">
                                <label for="nombre">Nombres</label>
                                <input type="text" class="form-control" id="nombre" required  v-model="ModoPago.nombre" @input="ModoPago.nombre = $event.target.value.toUpperCase()">
                            </div>
                        </div>
...
export default {
  ...
  methods: {
    ...
    cargarPermisosUser: function () {
      axios.get(`api/cargarPermisosUser/metpagos/${this.emailuser}`).then((res) => {
        if (res.data[0]){
    ...
    eliminar: function(ModoPago, index){
        const confirmacion = confirm(`Eliminar el modo de pago: ${ModoPago.nombre}`);
        if(confirmacion){
